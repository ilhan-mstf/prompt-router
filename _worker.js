    ...
    if (libMatch) {
      ...
      if (NON_EN_LANGS.has(langSub) && VALID_LIBS.includes(lib)) {
        ...
        const rewritten = new HTMLRewriter()
          ...
          .on('div#cats',                         { element: el => { if (localizedCatsHtml) el.setInnerContent(localizedCatsHtml, { html: true }); } })
          .on('div#prompt-cards',                 { element: el => { if (localizedCardsHtml) el.setInnerContent(localizedCardsHtml, { html: true }); } })
